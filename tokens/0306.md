# BRC-306: BSV-21 Fungible Tokens (Binary)

David Case / b-open-io (david.case@shruggr.cloud)

**Status: Draft.** Intended as the preferred encoding for new BSV-21 tokens. Wire format may still change until this BRC is marked stable.

*Normative content derived from the `bsv21-binary-docs` branch of [1sat-ordinals](https://github.com/BitcoinSchema/1sat-ordinals).*
...
This encoding is the **preferred path for new tokens**. It succeeds the widely deployed JSON / inscription form of BSV-21 ([BRC-305](./0305.md)), which remains supported. Same deploy-outpoint identity and the same UTXO + authority economics; different wire format, designed so contracts can participate.
...
| Legacy JSON encoding (existing inventory) | [BRC-305: BSV-21 JSON](./0305.md) |

This BRC does **not** define marketplace locks, overlay topic naming, or BRC-100 basket profiles.

### Legacy JSON inventory

The earlier inscription-based encoding is specified in [BRC-305](./0305.md). It is not required reading for implementing this document.
...
* For display and APIs, the canonical string form is `<txid>.<vout>` (64 hex chars in display txid byte order, **dot** separator). Converting between 36-byte form and that string reverses the 32 txid bytes only.
...
The referenced outpoint SHOULD hold image (or other display) bytes — commonly a B protocol file (`19HxigV4QyBv3tHpQVcUEQyq1pzZVdoAut`) or an ordinal inscription. `icon` is a pointer only — not embedded image data and not a URL. Wallets resolve the outpoint to fetch content. Unresolvable icons do not affect token validity.
...
* Otherwise: let \(I\) = sum of amounts on valid value inputs of this token; let \(O\) = sum of amounts on value outputs of this token.
  * Admit value outputs only when \(I \ge O\) (all value outs for the token, or none — all-or-nothing).
...
* **Draft status** — do not treat mainnet binary deploys as final until this BRC is numbered and marked stable.
...
5. [BRC-305: BSV-21 JSON (legacy)](./0305.md)
