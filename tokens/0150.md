...
This BRC defines the **provenance remittance** carried in [BRC-37](../outpoints/0037.md) `customInstructions` for outputs in the companion basket profile `p 1sat ordinals` ([BRC-303](./0303.md)). It specifies an offline-verifiable package that binds a current 1-sat outpoint (*tip*) to a 1Sat Ordinals *origin*, using BEEF or AtomicBEEF plus local verification of the first `ord` inscription envelope.
...
[BRC-303](./0303.md) documents how wallets hold and transfer 1Sat tips and how they attach display metadata. Those metadata fields remain **claims**: a malicious or buggy sender can attach another inscription’s origin to an unrelated 1-sat UTXO. Indexer-only checks help UX but introduce a trust dependency when the indexer is wrong, lagging, or unavailable.
...
Provenance is embedded as the `provenance` field of the basket profile’s `customInstructions` JSON object ([BRC-37](../outpoints/0037.md)). Display fields (`origin`, `name`, `app`) remain as defined by [BRC-302](./0302.md) and [BRC-303](./0303.md) and MUST NOT be treated as proven unless this remittance verifies.
...
| Whether unproven tips may be shown with sender `name`/`app` | [BRC-302](./0302.md) (claims); this BRC (when proof fails) |
