...
This document defines a **binary** prefix: token id and amount as fixed pushes (amount as a script number; id as a 36-byte outpoint in sighash preimage order), an optional payload, and an `OP_CODESEPARATOR` that ends the prefix. Wallets and contracts can build and check token outputs in script without assembling an inscription or parsing JSON.
...
```
<push token id | OP_0> <push amount | OP_0> OP_2DROP [<push payload> OP_DROP] OP_CODESEPARATOR <rest of script>
```
...
| Payload (optional) | Push of a CBOR map (see [Payload](#payload)), immediately followed by `OP_DROP`. Omitted entirely when there is no payload |
| `OP_DROP` | Drops the payload. Present if and only if the payload is present |
| `OP_CODESEPARATOR` | Ends the prefix. Always present |
...
* The payload is optional. When present it is exactly one push followed by exactly one `OP_DROP`, placed between the `OP_2DROP` and the `OP_CODESEPARATOR`. When absent, the `OP_CODESEPARATOR` immediately follows the `OP_2DROP`.
* Every value the prefix pushes is dropped before the `OP_CODESEPARATOR`, so the remainder of the script runs as if alone.
* The prefix ends at the first `OP_CODESEPARATOR`. Anything between the `OP_2DROP` and that separator other than a single `<push> OP_DROP` makes the script not a BSV-21 binary token output.
* A script is a BSV-21 binary token output when it begins with this layout, including a valid payload if one is present. There is no tag: the layout itself is the protocol marker. Id and amount define the role; payload **contents** do not affect balance or authority admission.
* A decoder consumes the prefix, through the `OP_CODESEPARATOR`; everything after it is ordinary locking-script content for whatever spend path the output uses.
...
### Signatures and the separator

Because the prefix ends with `OP_CODESEPARATOR`, any signature checked by the remainder of the script commits to a BIP-143 `scriptCode` that starts **after** the separator. The token id, amount and payload are therefore not part of the signed `scriptCode`. The signature still commits to the spent outpoint, so it cannot be replayed against a different token output.

* Wallets and signers spending a token output MUST compute the sighash with the `scriptCode` taken from after the prefix's `OP_CODESEPARATOR` (or after a later separator, if the remainder executes one).
* A contract in the remainder that reads its own locking script from the sighash preimage cannot see the prefix. Contracts that need their token id or amount must carry their own copies (for example in contract state) and write the prefix of any continuation output from those copies. Indexers admitting such outputs may check that the copies match the prefix.
...
* The payload is optional. When present, it MUST be a push of a CBOR map ([RFC 8949](https://www.rfc-editor.org/rfc/rfc8949.html)) followed by `OP_DROP`.
* Encoders SHOULD omit the payload when there is nothing to carry, and SHOULD use deterministic CBOR (RFC 8949 §4.2) when writing a map.
* A payload push that is not a CBOR map means the script is **not** a valid BSV-21 binary token output.
...
Optional metadata on the **deploy** output only. Later outputs normally omit the payload and inherit display data from deploy. Missing or malformed display fields do **not** invalidate the deploy. These keys have no protocol meaning on non-deploy outputs. Additional keys MAY be defined later.
...
```
OP_0 21000000 OP_2DROP <CBOR {"sym":"GOLD","dec":8}> OP_DROP OP_CODESEPARATOR
...
```
OP_0 OP_0 OP_2DROP <CBOR {"sym":"STABLE","dec":2}> OP_DROP OP_CODESEPARATOR
OP_DUP OP_HASH160 <pubkeyhash> OP_EQUALVERIFY OP_CHECKSIG
```

Value output (no payload):

```
<36-byte token id> 5000 OP_2DROP OP_CODESEPARATOR <locking script>
```

Authority output (no payload):

```
<36-byte token id> OP_0 OP_2DROP OP_CODESEPARATOR <locking script>
```
