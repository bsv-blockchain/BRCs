...
* A script is a BSV-21 binary token output when it begins with the layout for its role, including a valid payload push where the role requires one. There is no tag: the layout itself is the protocol marker. Id and amount define the role; payload **contents** do not affect balance or authority admission.
...
* On the wire the id is written in its **canonical** form:
  ...
  * **36 bytes** — txid in natural/internal byte order ‖ `uint32` little-endian vout, the same layout as outpoints in sighash preimages — when the deploy output index is non-zero. This only occurs for tokens first deployed under [BRC-161](./0161.md) at a non-zero output.
...
* A non-empty payload push that is not a CBOR map means the script is **not** a valid BSV-21 binary token output.
