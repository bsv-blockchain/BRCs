...
| Payload | Deploy and authority outputs only: `OP_0` (empty), or a push of a DAG-CBOR map (see [Payload](#payload)) |
...
* Only deploy and authority outputs carry the payload slot. It MUST be either `OP_0` (empty) or a push of a map encoded as [DAG-CBOR](https://ipld.io/specs/codecs/dag-cbor/spec/), followed by `OP_DROP`.
...
* DAG-CBOR gives every value exactly one encoding: definite lengths, string keys in a single canonical order, minimal integers, no undefined/NaN/Infinity, and no tags other than 42. Decoders MUST decode strictly and reject non-canonical input, so every indexer reads the same payload bytes the same way.
* Payload maps SHOULD NOT use tag 42 (CID links) unless a key defined for them requires it.
* A non-empty payload push that is not a valid DAG-CBOR map means the script is **not** a valid 1Sat token output.
