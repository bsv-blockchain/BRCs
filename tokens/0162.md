...
This document defines a **binary** prefix: token id and amount as fixed pushes (amount as a script number; id as a 36-byte outpoint in sighash preimage order), plus a payload slot on deploy and authority outputs only. Wallets and contracts can build and check token outputs in script without assembling an inscription or parsing JSON.
...
```
<36-byte token id> 5000 OP_2DROP <locking script>
```

Authority output (empty payload):

```
<36-byte token id> OP_0 OP_2DROP OP_0 OP_DROP <locking script>
```
