# BRC-301: 1Sat Ordinals — Inscription Envelopes

David Case / b-open-io (david.case@shruggr.cloud)

## Abstract

This BRC defines how **inscriptions** attach content to a [BRC-300](./0300.md) token: an `ord` **envelope** embedded in the locking script of a **1-satoshi** output.
...
See [BRC-300](./0300.md) for origin, sat ordering, and transfer.
...
* Chain identity (origin and transfers) is unchanged by envelopes — see [BRC-300](./0300.md).
...
Because envelopes live in **outputs** (not input witnesses), an inscription is identified by the outpoint of the output that carries it — the same `txid_vout` / `txid.vout` forms as in [BRC-300](./0300.md).
...
Transfers afterward follow [BRC-300](./0300.md) (1-satoshi input → 1-satoshi output under sat ordering). The envelope does not need to be repeated on every output in the chain unless the application re-inscribes.

### Metadata (MAP)

Optional application metadata MAY follow the inscription on the same output after `OP_RETURN`, using **Magic Attribute Protocol (MAP)** (protocol id `1PuQa7K62MiKCtssSLKy1kh56WWU7MtUR5`):

```
<lock> <ord envelope> OP_RETURN <MAP id> SET type ord name "…" …
```
...
Further keys and other ways to carry MAP (e.g. as an inscription field) are defined by application BRCs such as [BRC-304](./0304.md). Unknown keys MUST be ignored by readers that do not implement them.

Wallets that surface a display name (e.g. in BRC-100 `customInstructions`) SHOULD take it from MAP `name` when present.

## What this document does not cover

* Full MAP opcode encoding (see MAP / bitcom references)
* Collection schemas and in-envelope MAP field layout — [BRC-304](./0304.md)
...
Those may require [BRC-300](./0300.md), this envelope BRC, or both.
...
2. 1Sat Ordinals — <https://docs.1satordinals.com>
3. Ordinals inscriptions — <https://docs.ordinals.com/inscriptions.html>
