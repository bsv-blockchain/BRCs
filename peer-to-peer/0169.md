...
| `signature` | DER-encoded ECDSA signature by the key derived per BRC-42 and BRC-43 from the identity key that was the `subject` of the released handle's certificate — protocol ID `[2, "metanet handles envelope"]`, key ID `forward`, counterparty `anyone` (section 7.2) — over `SHA-256` of the UTF-8 encoding of `from`, `toIdentityKey`, `toHandle`, and `created`, in that order, each terminated by a single `LF` (`%x0A`) |
...
2. A client MUST verify the forwarding signature against the key derived from the identity key of the **previous** binding, which it can only do if it holds or can obtain that prior certificate. A forwarding record whose signature cannot be verified MUST be ignored.
...
**Publishing type metadata is an open consideration.** Deployed overlay infrastructure includes a certificate-type directory, reachable as the topic `tm_certmap` and the lookup service `ls_certmap` and wrapped by a registry client in several BSV SDKs, through which an operator publishes a type identifier together with a human-readable name, an icon, a description, and per-field descriptors. Its deployed wire format is not specified here; the proposed [BRC-184](../wallet/0184.md) separately explains optional metadata consumption and stewardship. The directory does not allocate or reserve a type, and its records are attributable to the operator that published them rather than to any authority over the type. Whether this document should recommend publishing there, and if so what an operator should publish and how a client should weigh a record it finds, is left open for review. Nothing in this section depends on it: the type values above are fixed by their derivation.
...
| `created` | ISO-8601 UTC timestamp |
| `signature` | DER-encoded ECDSA signature by the key derived per BRC-42 and BRC-43 from the identity key that is the `subject` of that binding's certificate — protocol ID `[2, "metanet handles envelope"]`, key ID `discover`, counterparty `anyone` (section 7.2) — over `SHA-256` of the UTF-8 encoding of `handle`, `domain`, `discoverable` as `true` or `false`, and `created`, in that order, each terminated by a single `LF` (`%x0A`) |

6. A host MUST honour the most recent valid directive it holds for a binding, and MUST reject one whose signature does not verify against the key derived from the current subject key.
...
2. `contentHash` is OPTIONAL. When present it MUST be the lowercase hex `SHA-256` of the plaintext that `content` encrypts, taken over the exact bytes the sender encrypted. It is metadata, so it is inside the signature preimage (item 3), which binds the plaintext to the sender's signature: anyone holding the plaintext can verify who wrote it from the envelope alone, without the recipient's key, while the messagebox still enforces policy without reading content. A recipient MUST reject an envelope whose decrypted content does not hash to a present `contentHash`. Whether to require the field is the recipient's own policy; an envelope without it is valid.
3. `signature` MUST be a DER-encoded ECDSA signature by the key derived from `sender.identityKey` per [BRC-42](../key-derivation/0042.md) and [BRC-43](../key-derivation/0043.md) under protocol ID `[2, "metanet handles envelope"]`, key ID `send`, and counterparty `anyone`, over `SHA-256` of the envelope canonicalized per RFC 8785 (JSON Canonicalization Scheme) with the `content` and `signature` members removed. A verifier derives the corresponding public key from `sender.identityKey` with the same constants. As with certificate signatures (section 4.1, Appendix A.3), the identity key never signs directly: a [BRC-100](../wallet/0100.md) wallet does not expose root-key signing, so a signature by the identity key itself could not be produced through the wallet interface. With counterparty `anyone` the level-2 grant is a single permission per application, and reusing one derived key across envelopes is safe (RFC 6979 nonces); a per-envelope key ID would add no unlinkability, since `sender.identityKey` is plaintext. Canonicalizing rather than signing the transmitted bytes means a messagebox may re-serialize the envelope without invalidating it, while any change to a metadata value is detected. A messagebox MUST reject an envelope whose signature does not verify.
4. `sender.handle` and `sender.domain` are a claim. A messagebox MAY resolve them, and a recipient client MUST resolve and verify them per section 5.7 before displaying the sender as a named, verified identity. Until then the sender is identified only by `identityKey`.
5. `recipient.tag` carries the subhandle tag verbatim, subject to section 3.3.
6. `payment` is `null` where no payment is attached.

#### 7.3 Binary encoding

An envelope MAY be carried as a [DAG-CBOR](https://ipld.io/specs/codecs/dag-cbor/spec/) map instead of a JSON object, for example in a [BRC-231](./0231.md) relay body. The keys are those of section 7.2, with these types:

| Member | Type |
| --- | --- |
| `metanetHandles` | tstr |
| `recipient` | `{ handle: tstr, tag?: tstr, domain: tstr }` |
| `sender` | `{ identityKey: bstr(33), handle?: tstr, domain?: tstr }` |
| `created` | tstr |
| `quoteId` | tstr, optional |
| `payment` | `{ derivationPrefix: bstr, derivationSuffix: bstr, protocol: bstr, satoshis: uint, beef: bstr }` or null |
| `contentHash` | bstr(32), optional |
| `content` | bstr, the BRC-78 serialization |
| `signature` | bstr, DER |

`signature` is made as section 7.2 specifies, over `SHA-256` of the DAG-CBOR encoding of the map with `content` and `signature` removed; DAG-CBOR is deterministic, so no canonicalization step is needed. A signature is over the encoding it was made in: a relay delivers an envelope in the encoding it was submitted in and does not transcode. Appendix A.7 gives the binary form of its envelope.
...
```json
{
  ...
  "contentHash": "76f88073e3498f542c57fb9a2133743d6f0c1f9c4632e786116568ccd393e35c",
  "content": "<BRC-78 encrypted payload, elided>",
  "signature": "304402202321ddef29a863e6ff2d03856a6eb1a1f10a058e8197e6a63499d5a31801c74a02207801f7755d4d303255f739e24363ae697bbc7c9dbbb83fb5e7b79cad45ec0e70"
}
```

The plaintext that `content` encrypts is the UTF-8 string `See you at conf2036 — crumbs`; `contentHash` is its `SHA-256`.

**Signing key.** Per section 7.2 the sender does not sign with its identity key directly. It signs with a BRC-42 key derived under protocol ID `[2, "metanet handles envelope"]`, key ID `send`, counterparty `anyone`, giving the BRC-43 invoice number:

```
2-metanet handles envelope-send
```

As in A.3, both sides compute the same shared secret because the counterparty is `anyone` (private key `1`). The derived signing public key, which a verifier computes from `sender.identityKey` and checks the signature against, is:

```
022b380d363a1da7db379d567ebd438ff34684d1ab5cc82d8de57868d873c69552
```
...
The same envelope in the binary encoding of section 7.3 (with an empty `beef` for the elided payment). The signature preimage — the map without `content` and `signature` — is 398 bytes:
...
and the signature, by the same derived key:
...
```json
{
  ...
  "signature": "3045022100ca5e7b3d97bd589c6b248f789ecef175f8d673631a5a6f653be0c9693ee1b9050220528235f3a2d27334afedcd25f745b1d445cbb746a07919addbf4dd4137f4b048"
}
```

The signing key is derived from the departing subject's identity key under `2-metanet handles envelope-forward`, counterparty `anyone`; the derived public key a verifier checks against is `0395cf990e3c71a8a803847f4632c662ba10432886304e5456415d298b7e9e57a1`. The preimage is the four values, each followed by `LF`:

```
@deggen@lkup.net\n033cef496cd596a9dab13b36335cf51383322056f118c91e71d80be69b87e5db66\n@deggen@nexus.example\n2027-01-15T00:00:00Z\n
```

Critically, this verifies against the key derived from `0359c5f3...`, the **old** identity key from A.1, and not against anything derived from the `lkup.net` certifier key. A host cannot forge a redirect for a handle it controls, which is the property section 4.3 requires. A client following this record must also apply section 4.4, since the identity key has changed.
