...
Wallets implementing this specification MUST expose the following method, in keeping with the [BRC-100](./0100.md) interface conventions:
...
Check 2 is **not** implied by check 3, and omitting it is the most likely way to build a non-conforming implementation. Some widely used libraries accept an x-coordinate numerically greater than the field prime, reduce it modulo `p` without signalling anything, and then report the reduced point as on-curve. An implementation that validates only with an on-curve test therefore accepts a point that was never validly encoded. The reference implementation cited below was written with this bug and a test caught it; see [Implementations](#implementations).
...
A reference implementation in Go, with a test suite establishing the properties this specification depends on, is available at:
