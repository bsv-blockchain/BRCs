# BRC-370: Race-Settled Collection Markets for Overlay Lookups and Message Boxes
...
- protocol ID `[2, "BRC-370 collection grant"]`
...
| Method | Path | Body |
| --- | --- | --- |
| `POST` | `/brc370/query` | Query object. Returns an attestation, optionally with payload. |
| `POST` | `/brc370/collect` | Collect request. Returns the canonical payload. |
| `GET` | `/brc370/params` | Unauthenticated. Returns the host's `t`, `K`, `floorFeeSats`, and supported query classes. |

A BRC-33 `listMessages` call MAY be answered as a `message-list` query class without a separate `/brc370/query` hop when the client includes the BRC-370 query fields in the existing body. Hosts that do not implement BRC-370 ignore those fields.
...
1. Advertise BRC-370 support from `/brc370/params` and, when using overlay advertisements, from the existing SHIP/SLAP records.
...
Single-host operators implementing BRC-33 today can add `/brc370/params` returning `t = 1`, `K = 1` and accept BRC-105 payment on `listMessages` without implementing gossip.
